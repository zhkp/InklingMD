...
| 2 | 没测到 —— ①infra 故障（server 起不来、场景缺失等）；②**判定覆盖不足**（`PERF_REQUIRE_COMPARISON=1` 下部分场景无基线 / 不可比 / **基线里没有可用指标**，tag 运行会红着报出来）；③**部分场景未测量**（单场景超时 / 失败、无采样落盘：已测场景照常判定，未测量场景在报告里单列 `UNMEASURED`） | 先修测量链路或补齐该档位基线，别当成"没回归" |
