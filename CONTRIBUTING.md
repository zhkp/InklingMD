...
- 绝对判定（不依赖基线，首次运行也生效）：滚动场景的帧间隔 p95 不得超过 `2 × 帧预算`，
  ...
  绝对判定带有**环境属性**——它衡量"这份文档在当前机器上能否跑满帧预算"，
  只有在具备高刷显示（`PERF_HEADED=1`）或解除 vsync 上限（`PERF_UNCAPPED=1`）时才有意义。
  **CI 默认关闭**（headless 锁 60Hz、runner 无 GPU 且负载波动大：同代码两轮实测 jankRate
  21.7% 与 <10%，衡量的是 runner 而不是代码）；需要绝对结论请在本地跑，或手动触发工作流时
  勾选 `absolute`。
