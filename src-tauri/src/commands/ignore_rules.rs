...
mod tests {
    ...
    /// 目录符号链接不被跟随：内容不进结果
    ///
    /// 回归时的真实暴露机制是**条目泄入**而非无限递归（#250）：`follow_links` 若被打开，
    /// 指向真实目录的链接（`linked`）会让其内容以链接路径重复出现在结果里——变异验证
    /// 证明断言能精确抓到 `linked/inside.md` 泄入；自指链接（`loop`）则由 walkdir 的
    /// 祖先环检测兜底、不会挂死。因此本测试的回归抓手是下方对结果集合的断言。
    ///
