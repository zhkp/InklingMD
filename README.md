...
- **v3.0.0** 主版本质量攻坚——**33 个 issue + 7 个 PR，61 个提交 / 113 文件（+8486−722）**，无新功能，全部投入数据安全、竞态治理、崩溃兜底、性能、安全与可访问性六大方向。三条贯穿原则：**失败要留下完整数据**（#146 写入回退改 4×50ms 原子重试、全败保留原件；#161 硬链接+删源闭合 TOCTOU 窗口、跨卷回退保留源原件）、**状态按最小粒度隔离**（#148/#149/#164 保存与退避从工作区级下沉到标签页/文件级）、**降级只针对真失败**（#172 慢启动只提示不卸载；#171 良性错误不再整页崩溃）。另含 #176 搜索预览字符级窗口防 OOM、#163 代次取消+分片并行、#168 Mermaid/KaTeX 动态导入（3.1MB 移出启动图）、#153 快照写入治理、#192 查找装饰增量、#162 asset scope 收缩与远程图 no-referrer、#188 五处键盘/ARIA 补齐。全套 **115 单测文件 745 用例 / 169 E2E / 59 Rust** 全绿。详见 `docs/v3.0.0 设计文档.md`
...
- **Haoze Wu** ([@TomGoh](https://github.com/TomGoh)) — 修复本地图片相对路径解析（PR #8）、中文句号字形修复建议（issue #9）、大型工作区按需加载与文件树优化（PR #15）、主题原生控件配色同步（PR #16）、打开文件保留侧边栏文件树（PR #17）、Release 增加 Linux amd64 构建（issue #13/PR #18）、多标签滚动/光标位置防串扰（issue #30/PR #35）、macOS E2E 平台按键兼容（issue #36/PR #37）。
  - **v3.0.0 特别贡献（9 个提交）**：在本次质量攻坚中独立承担了两块工作——**功能侧**：大纲（TOC）标题扫描防抖与标题变化实时刷新、链接对话框主题样式恢复与自定义配色生效、动态图片目录 ACL 权限开启；**测试侧**：为二进制编解码 / 二进制写入 / 桌面端图片源 / PNG 导出 / 退出保存（新增 `src/lib/useExitHandler.ts`）/ ACL 能力配置 / 工作区存储，以及 Rust `commands` 与 `lib.rs` 补上关键路径的真实测试覆盖，并把既有回归测试与运行时真实行为对齐。这些工作直接支撑了本版本 745 单测 / 169 E2E / 59 Rust 用例的通过。
- **zhkp** ([@zhkp](https://github.com/zhkp)) — 项目作者，主要开发与维护

### 致谢

v3.0.0 是本仓库迄今修复密度最高的一个版本——33 个 issue、7 个 PR、61 个提交。这份工作量里有相当一部分来自 **[@TomGoh](https://github.com/TomGoh)** 的主动投入：他不仅在功能侧修好了大纲实时性与链接对话框主题一致性，更补上了桌面端与持久化路径上长期缺失的测试覆盖——这些测试在过去几轮重构中反复暴露出"改动无人把关"的风险。特此致谢。
