...
| 3.1 编辑器内核 | 万行复杂文档滚动流畅（120Hz 满帧） | ✅ | v3.1.0 | issue #212（PR #213）：滚动路径零几何读取——锚点采样前移到切换指令触发（新增 `lib/wysiwyg-anchor-sampler` 注册表），outline 标题重建与 CM6 懒挂载批次移入滚动停歇窗口 |
| 3.5 测试质量 | #136 门禁用例 B 首跑 flaky 消除 | ✅ | v3.1.0 | issue #214（PR #215）：`buildLongDocInWysiwyg` fixture 退出过渡后改 `waitScrollConverged` 等双收敛（与用例 A 对齐），无 retries 全量 E2E 稳定 |
| 3.5 测试质量 | 编辑器性能回归防线（Benchmark 判定体系） | ✅ | v3.2.0 | issue #216（PR #231）：`pnpm run benchmark` 6 场景 × 4 档位 × 真实负载 fixture；三层判定——噪声门槛 3σ（按实测跨运行散布，`inputSyncMs` 3σ=1.25ms / `ttiMs` 3σ=241ms / vsync 量化的 `frameMs.p95` 仅 0.39ms）、判定分层（主指标可单独判 FAIL，派生指标需佐证，小量级指标含实测绝对地板）、两阶段复现（首轮超阈值只复测该场景，退出码 0/1/2）；基线身份 = 测量配置（`<profile>/<mode>/r<rounds>/` 路径隔离），滚动帧预算绝对目标仅定向模式产出 |
| 3.3 文件与工作区 | 工作区忽略规则统一与文件索引命令 | ✅ | v3.3.0 | issue #227（PR #240）：实测发现忽略黑名单有三份（`searchIgnore.ts` / `search.rs` / `commands/mod.rs`），且前端那份是死代码、`.gitignore` 完全不生效；新增 `commands/ignore_rules.rs` 收敛为唯一真值源（默认 14 项目录 + 工作区内 `.gitignore`（含嵌套）+ 点号前缀隐藏项，明确不启用 `.ignore` / git 全局排除），新增 `list_workspace_files`（上限 5 万 + `truncated`、路径字节序升序、平台原生分隔符），**代次改由 Rust 侧分配**（严格单调，「最新请求胜出」跨窗口成立，前端零状态）；1 万文件遍历 1.543s → 52ms、5 万 7.536s → 257ms（~29×，线性） |
| 3.3 文件与工作区 | 快速打开文件（Quick Open） | ✅ | v3.3.0 | issue #228（PR #242）：`Ctrl/Cmd+P` 面板，输入即过滤（文件名 / 路径 / 子序列），索引工作区级缓存（TTL 30s + stale-while-revalidate + `invalidateEpoch` 拦在途写回 + `refresh` 链递归消费），打分「已打开 +1000 / 最近打开 0~100 / 匹配档位 0~100 − 深度×2」且排序键全 code-unit 比较，渲染上限 200（不做虚拟滚动），combobox 无障碍语义 + 焦点陷阱 + IME 组字守卫，模态收敛为单一 `activeModal` 归并点；实测 5,000 文件索引 20.3ms、打分 1.8ms |
| 3.5 测试质量 | 索引遍历符号链接用例 | ✅ | v3.3.0 | #222 计划 §7.1 第 3 项（PR #243）：`directory_symlinks_are_not_followed`（含自指链接，被跟随即无限递归）与 `file_symlinks_are_treated_as_files`；环境守卫按「链接确实可用」判定（`symlink_file` 在无权限环境下的返回值不可靠，两种表现均实测到） |
| 3.2 其他 | 全局搜索遵循 `.gitignore` | ✅ | v3.3.0 | issue #227（PR #240）：忽略规则统一后，`venv/`、`vendor/`、`Pods/`、`__pycache__/` 等由工作区 `.gitignore` 排除，不再被搜索结果扫到（文件树忽略目录同步统一为 14 项，实际新隐藏的只有 `coverage/`） |

### 8.2 发布版本

- **v3.3.0** Quick Open 交付完成——（Epic #222 子项 1/2 #227 + 子项 2/2 #228，PR #240 / #242，另 #243 闭合计划 §7.1 清单；21 个提交 / 41 文件 +3707−213）。①**忽略规则唯一真值源（#227 / PR #240）**：忽略黑名单实际有三份（`searchIgnore.ts` 14 项 / `search.rs` 同 14 项 / `commands/mod.rs` 5 项子集）靠注释手工同步，前端那份全仓只被自己的测试 import（死代码），且 **`.gitignore` 完全不生效**（`venv/`、`vendor/`、`Pods/`、`__pycache__/` 里的 `.md` 会被全局搜索扫到）。新增 `commands/ignore_rules.rs` 收敛为**三项穷举**（默认 14 项目录 + 工作区内 `.gitignore`（含嵌套子目录）+ 点号前缀隐藏项），明确不启用 `.ignore` / git 全局排除 / `.git/info/exclude` / 祖先 `.gitignore`；`hidden(false)` + `filter_entry` 自行按点号判定（不复用 ignore crate 的 Windows 隐藏属性判定，否则带隐藏属性的普通文件会被连带过滤）；`.ignore(false)` 显式关闭并用例锁定（旧实现也不读，但防默认值将来变化）。新增 `list_workspace_files` 命令：上限 `MAX_INDEX_FILES = 50_000` + `truncated`、路径字节序升序、平台原生分隔符、`root` 为文件时不特判；**代次改由 Rust 侧分配**（评审裁决改机制而不是改注释——`fetch_max` + 各 webview 独立计数器 =「先到窗口永久胜出、后开窗口被饿死」，改为 `fetch_add(1) + 1` 严格单调，前端零状态；`INDEX_GENERATION` 与 `SEARCH_GENERATION` 刻意分离，避免「打开 Quick Open」取消在途全局搜索）。测试竞态在结构上消除（取消链路注入桩、代次函数接收计数器引用），**证据边界如实说明**：摘掉测试锁连跑 15 轮未复现失败，故依据是结构性而非统计性复现。行为变化：文件树忽略目录 5 → 14 项，**实际新隐藏的只有 `coverage/`**；全局搜索新增 `.gitignore` 过滤（结果减少，用户预期内）。性能：1 万文件 **1.543 s → 52 ms**、5 万 **7.536 s → 257 ms**（~29×，线性）。②**Quick Open 面板（#228 / PR #242）**：索引缓存层懒构建 + `INDEX_TTL_MS = 30_000` + stale-while-revalidate（过期**先返回旧结果**再后台重建）+ 失效 + 降级 + 单文件模式候选集；`invalidateEpoch` 世代号拦「在途请求落定后写回」（否则旧清单会以新 `builtAt` 被当成新鲜数据，TTL 内一直读到重命名前的路径），并额外标记 `stale` + 立刻补一次重建；`refresh` 链改为**递归消费**（只挂一层 `.then` 会在「TTL 后台重建在途 + 一次失效」时渲染中间版本且永远等不到最终数据）；两条 mint 路径统一到 `startBackgroundRebuild()` 在 mint 处兜底（防 `unhandledRejection`）；单文件模式改用 `commonParentDir()` 取最深共同父目录（`relativeToRoot` 对基准外路径返回 basename，会让 `/a/report.md` 与 `/b/report.md` 无法区分）。打分排序纯函数：`已打开(+1000) + 最近打开(0~100) + 匹配档位分(0~100) − 深度×2`，六档取最高不叠加，排序键 `score↓ → relPath(小写)↑ → 绝对路径↑` 且**禁 `localeCompare`**（保证跨平台 / 跨 locale 确定）；**修正计划书**的深度惩罚符号（原书写 `+`，会让越深的文件越靠前）。面板渲染上限 `MAX_RENDERED_RESULTS = 200` + 底部提示，**不做虚拟滚动**；无障碍采用 WAI-ARIA combobox 模式（焦点始终留输入框 + `aria-activedescendant` + `aria-label` + 焦点陷阱，**未复用 `useMenuA11y`**——它会夺走输入框焦点、打断连续输入）；**IME 组字守卫**（组字期间的 Enter / 方向键 / Esc 不是命令，否则拼音按 Enter 确认候选会同时打开文件并关掉面板）；模态状态由 6 个布尔量收敛为单个 `activeModal`（`requestModal` 唯一归并点、`resolveModalAction` 三分支），并把模态层提为 `modalLayer` 在两个分支都渲染（修掉禅模式下的全局静默锁，`ConflictDialog` 一并纳入）；快捷键 `quickOpen` 默认 `mod+p`（备选 `mod+shift+p` 同样被 CM emacs keymap 占用，按项目早已为同源 `Ctrl-n` 做过的过滤一并处理）。性能实测：5,000 文件索引中位数 **20.3 ms**（验收线「< 2s」约 100× 裕量）、5,000 候选打分 **1.8 ms**、渲染 ≤200 行、打字不触发磁盘 / 索引工作；**真机 DOM 渲染耗时无有效自动化手段量化，按计划属人工核对项，不宣称「已验证达标」**。③**测试补全（#243）**：闭合 #222 计划 §7.1 第 3 项，补目录符号链接（含自指链接）不被跟随与文件链接按文件处理的断言，环境守卫按「链接确实可用」判定。质量门禁：Vitest **130 文件 / 942 用例**、E2E **181**、Rust CI ubuntu 85 / windows 90 全绿（本机发版实测 90 通过 / 0 失败）、`tsc` 零错误、变异验证 **21 项全部精确命中**。详见 `docs/v3.3.0 设计文档.md`

- **v3.2.0** 性能回归防线基建——（#216，PR #231，17 个提交 / 50 文件 +6324−2；**`src/`、`src-tauri/` 零改动，不含产品行为变更**）。新增 `pnpm run benchmark`：6 场景（打开 / 输入 / 滚动 / 查找 / 标签切换 / 保存）× 4 档位（1k / 5k / 2 万 / 5 万行）× 真实负载 fixture（每 10 节一个 Mermaid、每 5 节一张图片，另配纯文本对照），median 与 p95 双行参与判定，原始采样落盘可独立复算。**①噪声门槛（3σ）**：基线为每指标保留最近 8 次运行的聚合值、参考值取历史中位数（滚动参考），变化未超 3σ 判 WARN 并标注原因而非 PASS——实测同代码跨运行散布 `inputSyncMs` 3σ=1.25ms（占基线 66%）、`saveMs` 32.5ms（61%）、`longTaskMs` 161ms（53%）、`ttiMs` 241ms（27%），而 vsync 量化的 `frameMs.p95` 仅 0.39ms（2.3%）：**能分辨多大差异是环境属性，固定百分比阈值必然误判**；历史不足 3 次时显式退化为「百分比 + 绝对地板」。**②判定分层**：主指标（`ttiMs`/`frameMs`/`switchMs`/`searchMs`/`saveMs`/`inputSyncMs`/`inputPaintMs` 及 `.p95`）可单独判 FAIL，派生指标需同场景有主指标佐证（否则 WARN 并说明），小量级指标另有实测绝对地板（`inputSyncMs` Δ≥1ms、`saveMs` Δ≥8ms，`.p95` 继承）。**③两阶段复现**：首轮超阈值只复测该场景，仍超判 FAIL、回落判 WARN，退出码 0/1/2（2 = 没测到）。**④绝对目标**：滚动场景帧间隔 p95 ≤ 2×帧预算、掉帧率 ≤10%，仅在 headed/uncapped 定向模式产出（headless 的 vsync 锁 60Hz 衡量的是机器忙不忙），判定资格随采样落盘。**⑤基线身份 = 测量配置**：`.perf-baseline/[local/]<profile>/<mode>/r<rounds>/` 路径隔离，headless 日常回归与 uncapped 120fps 定向验证互不覆盖；fixture（被测对象）不进路径，换文档即同路径重建历史。**工程**：新增 CI 工作流 `benchmark.yml`（PR 触发、不阻断合并、上传产物 + Job Summary）；三个纯模块划清职责（`judgment.js` 怎么判 / `comparability.js` 能不能比 / `report.mjs#baselinePath` 基线归谁），单测直接断言线上实现；单测 760 → 840 例，两个端到端测试用子进程真实跑 `report.mjs` 全流程并把断言打在编排交接点；每个修复均带变异验证（据此修掉绝对判定 FAIL 路径被复测筛选架空、`--scenario` 参数无消费点、基线历史跨模式污染三条真实断链）。质量门禁：Vitest **126 文件 / 869 用例**、E2E 171 用例、Rust 59 用例（Windows；ubuntu 55）、`tsc` 零错误、CI 双平台全绿；**09-16 重发（含 09-14~09-16 四轮评审加固：PR #232 / #235 / #237 / #239，关闭 #233 / #234 / #236 / #238）**：①**复测拆到独立 runner（#234）**——`benchmark` + `retest` 两个 job，同 runner 顺序复测过滤不掉「整台 runner 变慢」（两轮一起超阈值、穿过「连续 2 次」过滤），换 runner 后两轮才统计独立；无嫌疑的常规运行仍只有 1 个 job，另加 `PERF_SPLIT_RETEST` / `PERF_RETEST_ONLY` / `force_suspects` 便于确定性演练。②**会话标定负载（#236）**——每轮在页面内跑一段**与编辑器代码无关**的固定工作量（1500 节点 DOM 构建/样式/布局 + 2000 万步纯计算，实测 ≈36ms），进基线但**不参与判定**（机器慢不是代码回归），报告据此给**环境归因**：本次机器 vs 基线参考、是否超出历史范围（标定值分布即"机器档位"分布，双峰下 3σ 不适用），并做**两轮对账**——复测超范围→「复测环境异常，FAIL 未必成立」、首轮超范围而两台 runner 均复现→「回归在两台 runner 上复现」、无复测数据→明说"本次没有复测轮"；覆盖边界如实声明（CPU 与 DOM 构建/样式/布局，不含 IO/网络）。③**检出下限显式披露**（3σ ≥ 参考值 30% 的指标逐条列出，"行上的 PASS 不等于没问题"）与**整机漂移迹象披露**（判定不变，只披露）。④**判定覆盖口径修正**——只算相对行、空指标基线计入未覆盖、覆盖不足必须显式失败（不许"没比"伪装成"没回归"，也不许 `exit 1` 抢先掩盖）。⑤「判定分层」行名单改为从 `PRIMARY_METRICS` **派生**（原先手写，含一个不存在的 `inputMs`）。基线维护纪律写进 CONTRIBUTING：目标 8 点、每次发版后补 1 次、必须串行、不与 PR/发版检查并发（共享 runner 争用会让当次基准整体变慢）。质量门禁：Vitest **126 文件 869 用例**、E2E 171、Rust 59（CI 双平台全绿；本机 56 通过 + 3 个 symlink 用例受 Windows 开发者模式限制，非代码问题）、`tsc` 零错误。详见 `docs/v3.2.0 设计文档.md`
- **v3.1.0** 性能与测试可靠性修复——（#212 + #214，PR #213 + #215，3 个代码提交 / 10 文件 +747−82）。①**万行复杂文档滚动掉帧（#212）**：v2.8.0（#136 修复，PR #139 落地）的模式切换锚点缓存 `cacheTopPos` 在滚动路径每帧执行 `posAtCoords`→`elementFromPoint`，懒挂载持续脏化布局时每帧强制同步重排整篇文档（单次 6~32ms），120Hz 预算 8.33ms 被吃满，大文档滚动跌至 ~60fps。修复为**机制换血而非降频**——把锚点采样从「滚动路径每帧」前移到「切换指令触发时同步采样」：`setTabSourceMode` 翻转 sourceMode（React 塌陷容器）之前，经新增注册表 `lib/wysiwyg-anchor-sampler.ts` 同步采样（此刻编辑器可见，几何现场读可靠）；切换由离散事件触发、React 同步 flush，采样与消费之间无滚动事件可插入，锚点必然新鲜（滚动在途立即切换的竞争场景同样成立），语义与 #139 完全一致。滚动路径几何读取归零 + 停歇 150ms 防御保鲜；p99 尖刺治理：outline 失效检测与 `rebuildHeadingTops`（单次 27ms 强制布局）移滚动停歇 200ms 后、CM6 懒挂载 IO 入队 + 滚动让位后每帧挂载 1 个。实测（本机 60Hz 万行 1200 代码块文档 360 帧）：median 16.7ms 满 vsync、超 2×median 帧数 0。新增竞争场景回归 E2E `source-mode-scroll-race.spec.ts`（C1/C2，滚动在途切换的内容锚点级断言）。②**#136 门禁用例 B 首跑 flaky（#214）**：`buildLongDocInWysiwyg` fixture 退出源码模式后未等退出过渡 settle 收敛（只等 scrollHeight 撑起），用例 B 的滚底赋值被 settle 剩余帧拽回（main 33% / CI retries 掩盖）；改 `waitScrollConverged` 等 scrollTop+scrollHeight 双收敛（与用例 A 对齐），产品代码未动（settle 有 30 帧上限 + <1px 即止，属切换过渡既定行为）。用例 B `repeat-each=20` 全过（修复前 1/10 失败）。质量门禁：Vitest 116 文件 / 760 用例、E2E 171 用例、Rust 59 用例（Windows；ubuntu 55）、`tsc` 零错误、CI 双平台全绿。详见 `docs/v3.1.0 设计文档.md`
- **v3.0.0** 主版本质量攻坚——数据安全、竞态治理、崩溃兜底、性能、安全与可访问性六大方向（#146–#188、#192、#200 共 45 个 issue + 19 个 PR，61 个代码提交 / 113 文件 +8486−722）。**三条贯穿原则**：①失败要留下完整数据（宁可报错保留原件，不做「先删后建」的赌博）；②状态按最小粒度隔离（保存 / 退避 / 冲突从工作区级下沉到标签页与文件级）；③降级只针对真失败（慢启动不等于失败，良性错误不等于崩溃）。**数据安全（Rust 侧）**：#146 写入回退改为 4 次 × 50ms 原子重试、全败保留原件，消灭「先删目标再 rename」的数据丢失窗口；#161 文件移动优先「硬链接 + 删源」原子占用目标名闭合 TOCTOU 窗口（目标已存在时 `AlreadyExists` 原子失败），跨卷回退递归复制且失败保留源原件；#159 读取加 10MB 护栏与 `ENCODING_UNSUPPORTED`/`FILE_TOO_LARGE` 结构化错误；#166/#177/#200 收敛删除快照、重命名与在途读取的竞态链（读取完成后按落定路径归属 tab，堵住幽灵 tab）。**保存与冲突**：#148 `saving` 下沉到 `OpenTab`，只拦本 tab 重入，另存为挂起期间其他 tab 不再被静默吞掉；#149 `conflictPending` 期间暂停自动保存、退避计数按文件隔离；#164 `reloadFile` 统一清除冲突态；#170 重载决策前 flush 防抖、弹窗后复核 dirty；#186 冲突对话框补层级、Esc 与焦点管理。**编辑器与查找**：#178 空替换串改走 `tr.delete` 修复 `RangeError` 崩溃；#185 面板卸载 dispatch clear 清高亮残留；#151 `replace` 移出 effect 依赖 + 装饰按引用缓存；#192 单步事务走 `DecorationSet.map` 增量路径（窗外区间 mapping 平移、仅重扫变更窗口、坍缩空区间丢弃），多步回退全量；#152 选区非空输入右符号不再被吞；#172 重构 `useEditorFallback` 区分工厂抛错 / create 在途 / 异步失败，慢启动只提示不卸载（消除卸载—重建闪烁与双倍初始化）；#173 代码块语言加载竞态守卫；#174 卸载快照改用脱链安全读数。**性能**：#176 搜索预览改命中点字符级窗口（前后 120 + 片段封顶 200 字符），内嵌 base64 长行不再整行克隆 OOM；#163 引入搜索代次实现取消、去掉逐目录 canonicalize、按 CPU 分片并行且按片序合并保确定性；#160 截断对前端可见且恰好 5000 条不误报；#168 Mermaid/KaTeX 改为首次遇到节点才动态 `import()`，约 3.1MB vendor 移出启动加载图；#153 DeletedSnapshots 健康探测改 1 字节哨兵键、刷新改事件驱动，空列表零开销。**工作台**：#165 新增 `storageSync` 为 recents/bookmarks/expandedDirs 注册 storage 事件实现多窗口同步；#167 侧边栏折叠记忆与低窗口挤压；#147 单实例 open-file 从存活窗口寻址并 unminimize+set_focus；#150 另存为到已打开路径走 tab 合并（目标有未保存内容先确认、写盘窗口期新编辑不丢弃）；#187 激活 tab 滚入视野。**稳定性与安全**：#171 新增 `crash-guard.ts`，`isBenignGlobalError` 过滤 ResizeObserver 噪音与空 message，rejection 只记日志不换崩溃页；#162 asset scope 从 `$DATA`/`$TEMP` 收缩为 `$APPDATA`/`$APPLOCALDATA`，远程图加 `referrerpolicy="no-referrer"`。**可访问性**：#188 一次补齐 5 处（搜索开关 visually-hidden、保存指示器改真实 button、标签页 tablist 语义与方向键、菜单 `useMenuA11y` 与 Esc、文件树 `aria-level`），并加源级静态断言防回退。**交互**：#158 文件树非 md 行改用 `aria-disabled` 恢复 contextmenu 可达；#184 图片右键菜单幂等清理 + 模块级单例防泄漏叠加。**社区贡献**：@TomGoh（Haoze Wu）9 个提交完成大纲扫描防抖与实时刷新、链接对话框主题与自定义配色、动态图片目录 ACL 权限，并为二进制编解码 / 桌面图片源 / PNG 导出 / 退出保存（新增 `useExitHandler.ts`）/ 工作区存储与 Rust 命令补上关键路径测试。**测试**：Vitest 115 文件 745 用例、E2E 169 用例、Rust 59 用例（Windows；ubuntu 55，4 个 `#[cfg(windows)]` 用例不参与编译）、`tsc` 零错误、CI 双平台全绿，每个修复均配套变异验证。3 个需符号链接权限的 Rust 用例带无权限跳过守卫，不会误报失败。详见 `docs/v3.0.0 设计文档.md`
