...
1. **有回归却看不见**：性能没有基线，只能等用户实测反馈（v3.1.0 的 #212 万行文档掉帧即如此——120Hz 用户先察觉，60Hz 环境不易暴露）。
2. **有阈值也判不准**：共享 runner 上同一份代码的跨运行漂移极大。实测 5 次同代码运行的跨运行 3σ——`inputSyncMs` 1.25ms（占基线 66%）、`saveMs` 32.5ms（61%）、`longTaskMs` 161ms（53%）、`ttiMs` 241ms（27%）；而 vsync 量化的 `frameMs.p95` 只有 0.39ms（2.3%）。**"哪个指标能分辨多大差异"是环境属性**，15% 这类固定百分比本身就低于多数指标的噪声（开发中实测同一份代码被连续判 5 次"回归"）。
3. **尖刺漏检**：只看中位数会漏掉「每 10 帧一次 40ms 尖刺」这类周期性卡顿。

### 测量（`pnpm run benchmark`）

- **6 场景 × 4 档位**：打开 / 连续输入 / 滚动 / 编辑内查找 / 标签切换 / 保存；S=1k、M=5k、L=2 万、XL=5 万行。`quick` 档 16 个组合约 2.5 分钟。fixture 按真实负载生成（每 10 节一个 Mermaid 块、每 5 节一张真实图片），并为打开/滚动提供纯文本对照。
- **指标**：样本型 `ttiMs` / `inputPaintMs` / `inputSyncMs` / `frameMs` / `switchMs` / `searchMs` / `saveMs`（median 与 p95 双行参与判定）+ 派生标量 `longTaskMs` / `longTaskCount` / `longFrameCount` / `jankCount` / `jankRatePct` / `cls` / `heapDeltaMB`。
- **原始采样落盘**（含 `mode` 与绝对判定资格），判定可事后独立复算，不会因执行环境变化翻转结论。
- **高刷与真机口径**：`PERF_HEADED` / `PERF_UNCAPPED` / `PERF_FRAME_BUDGET_MS` / `PERF_DOC_FILE` / `PERF_REPEAT`。

### 判定（三层并行，缺一不可）

- **噪声门槛（3σ）**：基线为每个指标保留最近 8 次运行的聚合值，σ 由此估计；**参考值用历史中位数**（滚动参考，避免单点基线把整批指标判成改善/恶化）。变化未超 3σ 时判 WARN 并标注「变化在运行噪声内（3σ=…）」而不是 PASS——确实动了，但幅度不可行动。历史不足 3 次时显式退化为「百分比 + 绝对地板」并写在报告头部。
- **判定分层**：主指标（`ttiMs`/`frameMs`/`switchMs`/`searchMs`/`saveMs`/`inputSyncMs`/`inputPaintMs` 及其 `.p95`）可单独判 FAIL；派生指标需同场景有主指标佐证，否则 WARN 并标注原因。小量级指标另有**实测绝对地板**（`inputSyncMs` Δ≥1ms、`saveMs` Δ≥8ms 等，`.p95` 行继承）。
- **两阶段复现**：首轮超阈值 → 只复测该场景 → 仍超判 FAIL，回落判 WARN；退出码 `0`/`1`/`2`（2 = 没测到，别当成"没回归"）。
- **绝对目标**：滚动场景帧间隔 p95 ≤ 2×帧预算、掉帧率 ≤10%，**仅在定向模式（headed/uncapped）产出**（headless 的 vsync 锁 60Hz 衡量的是机器忙不忙，不是代码质量），判定资格随采样落盘。
- **基线身份 = 测量配置**：`.perf-baseline/[local/]<profile>/<mode>/r<rounds>/<id>.json` —— headless 日常回归与 uncapped 定向验证各自维护、互不覆盖；fixture（被测对象）不进路径，换文档即同路径重建历史。
- 控制台 WARN 摘要按成因分组（复测回落 / 未复测 / 无主指标佐证 / 运行噪声内 / 低于绝对地板）。

### 工程质量

- **新增 CI 工作流** `benchmark.yml`：PR 触发、**不阻断合并**，上传采样与基线产物并写入 Job Summary；`update_baseline` 手动触发用于重建基线。
- **单测 760 → 869 例**（126 文件），perf 相关 9 个文件覆盖判定策略、可比性矩阵、绝对判定资格、绝对失败路径、基线历史累积、CLI 参数映射；两个端到端测试用**子进程真实跑 `report.mjs` 全流程**，断言打在编排交接点（如 check 阶段的复测清单）而非只看终态。
- 每个修复均带**变异验证**（改坏被测行为 → 新测试必须失败），并据此修掉了三条真实断链：绝对判定的 FAIL 路径曾被复测筛选架空（用户跑 120fps 验证时超标却拿到绿灯）、`--scenario` 参数解析了却无消费点（argv 形式静默全量跑）、基线历史跨测量模式污染（σ 被污染后真实回归被判成抖动）。
- `CONTRIBUTING.md` 新增「性能 Benchmark」完整章节（判定规则、基线维护、可比性五维、噪声门槛）。

### 发布后加固（2026-09-14 ~ 09-16，**本版本为重发**）

> 09-13 首次发布的 v3.2.0 资产只到 PR #231。此后 benchmark 又经历四轮评审加固（PR #232 / #235 / #237 / #239，关闭 #233 / #234 / #236 / #238），
> 本次以**当前 `main` 重建 tag 与安装包**——版本号不变，基线数据一并重新播种。

**PR #232（09-14）——判定可信度**

- **检出下限显式披露**：报告列出 3σ ≥ 参考值 30% 的指标并注明「这些指标只能检出更大的变化，行上的 PASS 不等于没问题」——「假 FAIL 会被人发现，静默漏检不会」，宁可把"测不出来"说清楚。
- **整机漂移迹象披露**：互不相关的指标大面积同时变差时显式提示（判定不变，只披露）——共享 runner 上「机器慢」与「代码慢」无法仅凭报告区分，必须先说清楚（#234 实测一次慢会话里 88% 的相对行同时变差、中位 Δ +18.7%）。
- **判定覆盖口径修正**：覆盖只算**相对行**（绝对行不构成相对覆盖）；空指标基线同样计入"未覆盖"；覆盖不足时必须显式失败——不允许"没比"伪装成"没回归"，也不允许 `exit 1` 抢先掩盖覆盖问题。

**PR #235（09-15，#233 / #234）——复测独立性与基线维护**

- **复测拆到独立 runner（#234）**：`benchmark` + `retest` 两个 job——首轮只做测量 + check，检出疑似回归才把清单移交**新 runner** 复测并出 final 判定。同 runner 顺序复测无法过滤"整台 runner 变慢"的会话（两轮一起超阈值、穿过「连续 2 次」过滤）；换 runner 后两轮才统计独立。无嫌疑的常规运行仍只有 1 个 job。
- 编排抽成纯函数 `planRetestPhases` + 6 例单测；新增 `PERF_SPLIT_RETEST` / `PERF_RETEST_ONLY` / `force_suspects`（可确定性演练移交路径，不必等真实回归）。
- **基线维护纪律（#233）**：目标 8 点、**每次发版后补 1 次**、必须串行、**不要与 PR/发版检查并发**（共享 runner 争用会让当次基准整体变慢）；quick 基线补到上限 8 点。
- **播种产物名写清**：检出疑似回归时基线更新发生在 retest job，须取 `perf-benchmark-<profile>-retest`——取 job1 产物等于本次播种静默丢失（两个 job 的 Job Summary 均就地提示）。

**PR #237（09-16，#236）——环境归因**

- **会话标定负载**：每次运行在页面内跑一段**与编辑器代码无关**的固定工作量（1500 个节点的 DOM 构建/样式/布局 + 2000 万步纯计算，本机实测 ≈36ms），落到 `probeMs` / `probeLayoutMs` / `probeCpuMs` 并进基线。
- 它**进基线但不参与判定**（机器变慢不是代码回归）：报告据此给出**环境归因**——本次机器 vs 基线参考、是否超出历史范围（实测标定值分布即"机器档位"分布，双峰下 3σ 不适用，改判"是否超出历史范围"）。
- **两轮对账**：复测 probe 超范围 → 「复测环境异常，FAIL 未必成立」；首轮超范围而复测在范围内且 FAIL 复现 → 「回归在两台 runner 上复现」；无复测数据 → 明确写"本次没有复测轮"（不无中生有）。
- **覆盖边界如实声明**：只覆盖 CPU 与 DOM 构建/样式/布局，**不含 IO / 网络**。
- 用 #234 那次慢会话的真实产物回放验证：历史原样时报告无任何环境归因，注入慢标定值后即给出「复测环境异常，FAIL 未必成立」。

**PR #239（09-16，#238）——文案与白名单一致**

- 报告「判定分层」行的主指标名单改为从 `PRIMARY_METRICS` **派生**（原先手写，含一个不存在的 `inputMs`），并加"名单逐字等于白名单"的断言防漂移。

### 统计口径与测试

- 版本范围：`git diff --stat v3.1.0..main` → **86 个文件（+12817 / −8）**：40 个基线数据、34 个 tests、2 个 docs、`CONTRIBUTING.md` / `README.md` / `vitest.config.ts` / `package.json` 各 1；`src/` **零改动**，`src-tauri/` 仅 3 处版本号（`Cargo.toml` / `Cargo.lock` / `tauri.conf.json` 各 1 行）→ **不含产品行为变更**。
- 质量门禁：Vitest **126 文件 / 869 用例**、Playwright E2E 171 用例、Rust `cargo test` 59 用例（Windows；本机 56 通过 + 3 个 symlink 用例受「未开启开发者模式」限制——测试守卫失效，非代码问题，CI 全绿）、`tsc --noEmit` 零错误、CI 双平台全绿。
- 使用入口：`pnpm run benchmark`；详见 `docs/v3.2.0 设计文档.md`。
